...
public class CaveEntrance extends Cave {
    ...
    /**
     * Returns the cavenEntrace's cave. Probably to access it's deck.
     *
     ...
     *
     * @return The state of the cave entrance, locked or not.
     */
     ...
     *
     * @param caveParam A cave in parameter just so that the caveEntrance can
     * access the deck.
     */
    ...
    /**
     * Tells the explorers the last treasure they just discovered.
     *
    ...
    /**
     * Returns True if the explorers have already encounters a same Hazard tile
     * in their last path.
     *
     * @return True if the explorers have already encounters a same Hazard tile
     * in their last path.
     */
    ...
    /**
     * Removes the gems of the last tile.
     */
    public void makeLastTileExplored() {
        if (path.size() > 0 && path.get(path.size() - 1) instanceof Treasure) {
     ...
     *
     * @param explorers All the exploring explorers willing to go back to the
     * camp.
     */
    public void returnToCamp(List<Explorer> explorers) {
        if (this.getPath().size() - 2 > 0) {
